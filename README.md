...
Windows + 项目内 `.venv` 环境有两点和上面的 Linux 默认说明不同：

```powershell
# 前端如果直接访问 http://127.0.0.1:8765，需要用根路径重新构建
cd webui/frontend
$env:WEBUI_BASE="/"
pnpm i
pnpm build
...
# mitmproxy 12.x 与新版 pydantic 的 typing-extensions 约束会冲突，单环境安装时先收敛版本
.\.venv\Scripts\python.exe -m pip install "pydantic>=2.6,<2.12" "typing-extensions==4.14.0" tzdata
.\.venv\Scripts\python.exe -m pip check
```

WebUI 的系统检查里，`xvfb-run` 是 Linux 虚拟显示工具。Windows 不需要安装它；本项目的 WebUI 会在 Windows 上直接运行 `pipeline.py`。
Windows 还需要 `tzdata` 提供 `America/Chicago` 这类 IANA 时区名，否则 Python 的 `zoneinfo` 可能报 `ZoneInfoNotFoundError`。
...
pip install requests curl_cffi playwright "camoufox[geoip]" browserforge mitmproxy pybase64
...
如果你的主环境是项目目录里的 Windows `.venv`，可以把 ML 环境也放在项目目录，避免污染主依赖：

```powershell
.\.venv\Scripts\python.exe -m venv .\.venv-ctfml
.\.venv-ctfml\Scripts\python.exe -m pip install torch transformers opencv-python pillow numpy

# 运行独立 hCaptcha 求解器时使用这个 Python
.\.venv-ctfml\Scripts\python.exe CTF-pay\hcaptcha_auto_solver.py <bridge-url>
```
...
Windows 下没有 `xvfb-run`，直接用项目虚拟环境的 Python：

```powershell
.\.venv\Scripts\python.exe pipeline.py --config CTF-pay\config.paypal.json --paypal
.\.venv\Scripts\python.exe pipeline.py --config CTF-pay\config.paypal.json --paypal --daemon
```
